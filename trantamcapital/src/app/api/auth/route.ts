...
export async function GET(request: Request) {
  ...
  if (code) {
    ...
    if (data.access_token) {
      // Redirect popup to admin page — CMS auto-init detects token in hash
      const redirectTarget = `${origin}/admin#access_token=${data.access_token}&token_type=bearer`;
      return new Response(
        `<!DOCTYPE html><html><head><meta http-equiv="refresh" content="0;url=${redirectTarget}"></head><body><script>window.location.replace("${redirectTarget}");\x3c/script></body></html>`,
